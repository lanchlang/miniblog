...
func VerifyCaptcha(ctx *gin.Context) {
	var form VerifyCaptchaForm
	// This will infer what binder to use depending on the content-type header.
	if err := ctx.ShouldBindJSON(&form); err != nil {
		ctx.JSON(http.StatusBadRequest, gin.H{"error": "数据错误"})
		return
	}
	verifyResult := base64Captcha.VerifyCaptcha(form.Id, form.Value)
	if verifyResult {
...
type RegisterWithEmailForm struct {
	Username string `validate:"min=1,max=20,regexp=^[a-zA-Z0-9\u4E00-\u9FFF]*$" json:"username" form:"username" binding:"required"`
	Email    string `validate:"min=5,max=30,regexp=^([A-Za-z0-9])+@([A-Za-z0-9])+\\.([A-Za-z]{2\\,4})$" json:"email" form:"email" binding:"required"`
	Password string `validate:"min=8,max=20,regexp=^[A-Za-z0-9]{8\\,20}" json:"password" form:"password" binding:"required"`
}
...
func RegisterWithEmail(ctx *gin.Context) {
	var form RegisterWithEmailForm
	// This will infer what binder to use depending on the content-type header.
	if err := ctx.ShouldBindJSON(&form); err != nil {
		ctx.JSON(http.StatusBadRequest, gin.H{"error": "数据错误"})
		return
	...
	exist, err := model.Exist(&model.User{}, "username", form.Username)
	if err != nil {
		ctx.JSON(http.StatusInternalServerError, gin.H{"error": "暂时不能服务"})
		return
	}
	...
	exist, err = model.Exist(&model.User{}, "email", form.Email)
	if err != nil {
		ctx.JSON(http.StatusInternalServerError, gin.H{"error": "暂时不能服务"})
		return
	}
	...
	mailBody,err := generateVerifyBodyContent("template/email_verify.html",verifyCode, form.Email)
	if err!=nil{
		ctx.JSON(http.StatusInternalServerError, gin.H{"error": "暂时不能服务，请稍后再试"})
		return
	}
	err = util.SendVerifyMail(form.Email, mailHeader, mailBody)
	if err != nil {
		ctx.JSON(http.StatusInternalServerError, gin.H{"error": "暂时不能服务，请稍后再试"})
		return
	}
...
type SendCaptchaToPhoneForm struct {
	Phone string `validate:"min=5,max=13,regexp=^1[34578]\\d{9}$" json:"phone" form:"phone" binding:"required"`
}

//发送手机验证码
func SendCaptchaToPhone(ctx *gin.Context) {
	//TODO:添加一些验证的数据，不能让机器发送
	var form SendCaptchaToPhoneForm
	// This will infer what binder to use depending on the content-type header.
	if err := ctx.ShouldBindJSON(&form); err != nil {
		ctx.JSON(http.StatusBadRequest, gin.H{"error": "数据错误"})
		return
	...
	err := sendMessageCaptcha(form.Phone, verifyCode)
	if err != nil {
		ctx.JSON(http.StatusInternalServerError, gin.H{"error": "暂时不能服务"})
		return
	}
...
type VerifyPhoneCaptchaForm struct {
	Phone   string `validate:"min=5,max=13,regexp=^1[34578]\\d{9}$" json:"phone" form:"phone" binding:"required"`
	Captcha string `validate:"min=4,max=9,regexp=^\\d{4\\,9}$" json:"captcha" form:"captcha" binding:"required"`
}

//验证手机及其验证码
//TODO:记录错误到log
func VerifyPhoneCaptcha(ctx *gin.Context) {
	var form VerifyPhoneCaptchaForm
	// This will infer what binder to use depending on the content-type header.
	if err := ctx.ShouldBindJSON(&form); err != nil {
		ctx.JSON(http.StatusBadRequest, gin.H{"error": "数据错误"})
		return
	...
	err := phoneCaptcha.GetPhoneCaptcha(form.Phone, form.Captcha)
	if err != nil {
		ctx.JSON(http.StatusInternalServerError, gin.H{"error": "暂时不能服务"})
		return
	}
...
func RegisterWithPhone(ctx *gin.Context) {
	var form RegisterWithPhoneForm
	// This will infer what binder to use depending on the content-type header.
	if err := ctx.ShouldBindJSON(&form); err != nil {
		ctx.JSON(http.StatusBadRequest, gin.H{"error": "数据错误"})
		return
	...
	exist, err = model.Exist(&model.User{}, "phone", form.Phone)
	if err != nil {
		ctx.JSON(http.StatusInternalServerError, gin.H{"error": "暂时不能服务"})
		return
	}
...
func RequestResetPasswordThroughEmail(ctx *gin.Context) {
	var form RequestResetPasswordThroughEmailForm
	// This will infer what binder to use depending on the content-type header.
	if err := ctx.ShouldBindJSON(&form); err != nil {
		ctx.JSON(http.StatusBadRequest, gin.H{"error": "数据错误"})
		return
	...
	exist, err := model.Exist(new(model.User), "email", form.Email)
	if err != nil {
		ctx.JSON(http.StatusInternalServerError, gin.H{"error": "暂时不能服务"})
		return
	}
	...
	verifyCode := util.Hash(time.Now().String()) //生成验证码
	mailHeader := "修改密码"
	mailBody,err := generateVerifyBodyContent("template/reset_password_through_email.html",verifyCode, form.Email)
	if err != nil {
		ctx.JSON(http.StatusInternalServerError, gin.H{"error": "暂时不能服务，请稍后再试"})
		return
	}
...
type ResetPasswordForm struct {
	Password string `validate:"min=8,max=20,regexp=^[A-Za-z0-9]{8\\,20}" json:"password" form:"password" binding:"required"`
	TokenStr string  `json:"token" form:"token" binding:"required"`
}
...
func ResetPassword(ctx *gin.Context) {
	...
	claims, err := util.GetClaims(form.TokenStr)
	if err != nil {
...
type RequestResetPasswordThroughPhoneForm struct {
	Phone string `validate:"min=5,max=13,regexp=^1[34578]\\d{9}$" json:"password" form:"password" binding:"required"`
}

func RequestResetPasswordThroughPhone(ctx *gin.Context) {
	var form RequestResetPasswordThroughPhoneForm
	// This will infer what binder to use depending on the content-type header.
	if err := ctx.ShouldBindJSON(&form); err != nil {
		ctx.JSON(http.StatusBadRequest, gin.H{"error": "数据错误"})
		return
	...
	exist, err := model.Exist(new(model.User), "phone", form.Phone)
	if err != nil {
		ctx.JSON(http.StatusInternalServerError, gin.H{"error": "暂时不能服务"})
		return
	}
	...
	//返回token
	tokenStr, err := util.GenToken(60*10, form.Phone)
	if err != nil {
		ctx.JSON(http.StatusInternalServerError, gin.H{"error": "暂时不能服务"})
		return
	}
...
func LoginWithEmailAndPassword(ctx *gin.Context) {
	...
	var form LoginWithEmailAndPasswordForm
	// This will infer what binder to use depending on the content-type header.
	if err := ctx.ShouldBindJSON(&form); err != nil {
		ctx.JSON(http.StatusBadRequest, gin.H{"error": "数据错误"})
		return
	...
	//查找邮箱是否存在，如果不存在，则返回并说明
	user := model.User{}
	err = user.FindUserByEmailAndPassword(form.Email, form.Password)
	if err != nil {
		ctx.JSON(http.StatusInternalServerError, gin.H{"error": "暂时不能服务"})
		return
	}
...
func LoginWithPhoneAndPassword(ctx *gin.Context) {
	...
	var form LoginWithPhoneAndPasswordForm
	// This will infer what binder to use depending on the content-type header.
	if err := ctx.ShouldBindJSON(&form); err != nil {
		ctx.JSON(http.StatusBadRequest, gin.H{"error": "数据错误"})
		return
	...
	if err := validator.Validate(form); err != nil {
		ctx.JSON(http.StatusBadRequest, gin.H{"error": "请输入正确的电话号码和密码"})
		return
	}
	//查找邮箱是否存在，如果不存在，则返回并说明
	user := model.User{}
	err = user.FindUserByPhoneAndPassword(form.Phone, form.Password)
	if err != nil {
		ctx.JSON(http.StatusInternalServerError, gin.H{"error": "暂时不能服务"})
		return
	}
...
type LoginWithPhoneAndCaptchaForm struct {
	Phone string `validate:"min=5,max=13,regexp=^1[34578]\\d{9}$" json:"phone" form:"phone" binding:"required"`
	Code  string `validate:"regexp=^\\d{4}$" json:"code" form:"code" binding:"required"`
}
...
func LoginWithPhoneAndCaptcha(ctx *gin.Context) {
	...
	var form LoginWithPhoneAndCaptchaForm
	// This will infer what binder to use depending on the content-type header.
	if err := ctx.ShouldBindJSON(&form); err != nil {
		ctx.JSON(http.StatusBadRequest, gin.H{"error": "数据错误"})
		return
	...
	if err := captcha.GetPhoneCaptcha(form.Phone, form.Code); err != nil {
		ctx.JSON(http.StatusInternalServerError, gin.H{"error": "暂时不能服务"})
		return
	}
	if captcha.Id <= 0 {
	...
	user := model.User{}
	if err := model.GetOneByKey(&user, "phone", form.Phone); err != nil {
		ctx.JSON(http.StatusInternalServerError, gin.H{"error": "暂时不能服务"})
		return
	}
	if user.Id <= 0 {
...
type LoginWithUsernameAndPasswordForm struct {
	Username string `validate:"min=1,max=20,regexp=^[a-zA-Z0-9]{1\\,20}$" json:"username" form:"username" binding:"required"`
	Password string `validate:"min=8,max=20,regexp=^[A-Za-z0-9]{8\\,20}" json:"password" form:"password" binding:"required"`
}
...
func LoginWithUsernameAndPassword(ctx *gin.Context) {
	...
	var form LoginWithUsernameAndPasswordForm
	// This will infer what binder to use depending on the content-type header.
	if err := ctx.ShouldBindJSON(&form); err != nil {
		ctx.JSON(http.StatusBadRequest, gin.H{"error": "数据错误"})
		return
	...
	//查找邮箱是否存在，如果不存在，则返回并说明
	user := model.User{}
	err = user.FindUserByUsernameAndPassword(form.Username, form.Password)
	if err != nil {
		ctx.JSON(http.StatusInternalServerError, gin.H{"error": "暂时不能服务"})
		return
	}
